...
hardware_interface::CallbackReturn IntBrainHardware::on_init(
    ...
    cfg_.is_feedforward_ =
        info_.hardware_parameters["is_feedforward"] == "true" ? true : false;

    // IMU settings
    cfg_.imu_usage = info_.hardware_parameters["imu_usage"] == "true";
    cfg_.imu_sensor_fusion =
        info_.hardware_parameters["imu_sensor_fusion"] == "true";
    cfg_.imu_sensor_fusion_frequency =
        std::stoul(info_.hardware_parameters["imu_sensor_fusion_frequency"]);

    // Encoder settings
    cfg_.encoder_usage = info_.hardware_parameters["encoder_usage"] == "true";
    cfg_.encoder_velocity_calculation =
        info_.hardware_parameters["encoder_velocity_calculation"] == "true";
    cfg_.encoder_velocity_calculation_frequency = std::stoul(
        info_.hardware_parameters["encoder_velocity_calculation_frequency"]);
    ...
    cfg_.motor_current_meas_on_off =
        info_.hardware_parameters["motor_current_meas_on_off"] == "true";
    cfg_.motor_current_meas_frequency =
        std::stoul(info_.hardware_parameters["motor_current_meas_frequency"]);

    // Battery voltage measurement settings
    cfg_.battery_voltage_meas_on_off =
        info_.hardware_parameters["battery_voltage_meas_on_off"] == "true";
    cfg_.battery_voltage_meas_rate =
        std::stoul(info_.hardware_parameters["battery_voltage_meas_rate"]);
    ...
    if (motor_control_mode == "effort") {
        ...
        RCLCPP_FATAL(rclcpp::get_logger("IntBrainHardware"),
                     "Invalid motor control mode: %s. Supported modes are: "
                     "effort, feed_forward, pid_feed_forward, simple_pid",
                     motor_control_mode.c_str());
        return hardware_interface::CallbackReturn::ERROR;
    }

    cfg_.motor_controller_frequency =
        std::stoul(info_.hardware_parameters["motor_controller_frequency"]);
    ...
    for (const hardware_interface::ComponentInfo& joint : info_.joints) {
        ...
        RCLCPP_INFO(rclcpp::get_logger("IntBrainHardware"),
        ...
        float kd_filter_coeff =
            std::stof(joint.parameters.at("kd_filter_coeff"));
        float ff0 = std::stof(joint.parameters.at("ff0"));
        float ff1 = std::stof(joint.parameters.at("ff1"));
        motors[index++].setup(joint.name, kp, ki, kd, kd_filter_coeff, ff0,
                              ff1);
        RCLCPP_INFO(rclcpp::get_logger("IntBrainHardware"),
                    "Motor '%s' initialized with kp: %f, ki: %f, kd: %f, ff0: "
                    "%f, ff1: %f",
...
IntBrainHardware::export_state_interfaces() {
    ...
    for (auto& sensor : info_.sensors) {
        if (sensor.name == "imu_sensor") {
            ...
            state_interfaces.emplace_back(hardware_interface::StateInterface(
                ...
                &imu_.linear_acceleration_[0]));
            state_interfaces.emplace_back(hardware_interface::StateInterface(
                sensor.name, "linear_acceleration.y",
                &imu_.linear_acceleration_[1]));
            state_interfaces.emplace_back(hardware_interface::StateInterface(
                sensor.name, "linear_acceleration.z",
                &imu_.linear_acceleration_[2]));
        }
    }
...
IntBrainHardware::export_command_interfaces() {
...
hardware_interface::CallbackReturn IntBrainHardware::on_configure(
    ...
    // Battery voltage measurement settings
    // // TODO (eccentricOrange): Battery config parsing is not implemented in
...
hardware_interface::return_type IntBrainHardware::read(
    ...
    if (comms_.req_data(REQUEST_IMU_PROCESSED, imu_data) != 0) {
        RCLCPP_ERROR(rclcpp::get_logger("IntBrainHardware"),
                     "Failed to read IMU data: ORIENTATION");
        return hardware_interface::return_type::ERROR;
    }
    ...
    if (comms_.req_data(REQUEST_ENCODER_POSITIONS, encoder_positions) != 0) {
        RCLCPP_ERROR(rclcpp::get_logger("IntBrainHardware"),
                     "Failed to read encoder positions");
        return hardware_interface::return_type::ERROR;
    }
    for (size_t i = 0; i < info_.joints.size(); ++i) {
    ...
    if (comms_.req_data(REQUEST_ENCODER_VELOCITIES, encoder_velocities) != 0) {
        RCLCPP_ERROR(rclcpp::get_logger("IntBrainHardware"),
                     "Failed to read encoder velocities");
        return hardware_interface::return_type::ERROR;
    }
    for (size_t i = 0; i < info_.joints.size(); ++i) {
        if (cfg_.is_feedforward_) {
...
hardware_interface::return_type int_brain_hardware ::IntBrainHardware::write(
    ...
    for (const auto& motor : motors) {
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("IntBrainHardware"),
    ...
    if (comms_.send_data(MOTOR_DESIRED_RPMS, motor_commands) != 0) {
        RCLCPP_ERROR(rclcpp::get_logger("IntBrainHardware"),
                     "Failed to write motor commands");
        return hardware_interface::return_type::ERROR;
    }
